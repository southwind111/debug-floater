...
class GaussianTracker:
    def __init__(self, model, out_dir="logs", log_interval=500):
        self.model = model
        ...
        # !self.perturb_log = defaultdict(list)
    ...
    def track_state(self):
        self.iteration += 1
        full_log = (self.iteration % self.log_interval == 0)
        ...
        if not full_log:
            for i, gid in enumerate(ids):
                if gid not in self.prev_rgb_dict:
                    self.prev_rgb_dict[gid] = rgbs[i]
                if gid not in self.prev_pos_dict:
                    self.prev_pos_dict[gid] = positions[i]
            return
        ...
        #     neighbor_dists[gid] = None
            ...
            record = {
                ...
                "prune_reason": self._get_prune_reason(gid,hit_counts[i])
            }
            self.logs[gid].append(record)
            if len(self.logs[gid]) > 10:
                self.logs[gid].pop(0)
        ...
        for gid in self.active_ids:
            ...
            if reasons:
                ...
                if suspicious_score >= 1.0:
                    ...
                    if len(self.suspicious[gid]) > 1:
                        self.suspicious[gid].pop(0)
        del features_dc, features_rest, features_all, dirs, rgbs, positions, hit_counts, ids_tensor
    ...
    def flush_log(self):
        ...
        serializable_logs = {
            ...
            # "perturb_log": _to_serializable(self.perturb_log)
        }
    ...
    def _get_prune_reason(self, gid,hit_count):
        reasons = []
        ...
        return reasons
    def inject_vgs_perturbation(self):
        """对 suspicious 点注入扰动以促使其从错误收敛中跳脱"""
        import torch
        import random
        import numpy as np
        ...
        for gid, log_list in self.suspicious.items():
            if not log_list:
                continue
            last_log = log_list[-1]
            reasons = set(last_log.get("prune_reason", []))
            if not reasons & {"color_jitter", "fast_motion", "elongated", "isolated", "low_visibility"}:
                continue

            idx_tensor = (self.model._ids == gid).nonzero(as_tuple=True)[0]
            if idx_tensor.numel() == 0:
                continue  # 已被剔除
            idx = idx_tensor.item()

            # 动态扰动强度（根据suspicious_score）
            suspicious_score = last_log.get("suspicious_score", 1.0)
            k = 0.5  # 控制扰动强度增长速度
            strength = 1 - np.exp(-k * suspicious_score)
            strength = min(strength, 1.0)

            # 添加扰动
            with torch.no_grad():
                pos_perturb = strength * 0.01 * torch.randn_like(self.model._xyz[idx])
                scale_perturb = 1.0 + strength * 0.1 * torch.randn_like(self.model._scaling[idx])
                dc_perturb = strength * 0.02 * torch.randn_like(self.model._features_dc[idx])
                rest_perturb = strength * 0.02 * torch.randn_like(self.model._features_rest[idx])

                self.model._xyz[idx] += pos_perturb
                self.model._scaling[idx] *= scale_perturb
                self.model._features_dc[idx] += dc_perturb
                self.model._features_rest[idx] += rest_perturb
