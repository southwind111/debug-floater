...
from tracker import GaussianTracker
try:
...
try:
    ...
    SPARSE_ADAM_AVAILABLE = Tru
except:
    SPARSE_ADAM_AVAILABLE = False

def training(dataset, opt, pipe, testing_iterations, saving_iteratio#
# Copyright (C) 2023, Inria
# GRAPHDECO research group, https://team.inria.fr/graphdeco
# All rights reserved.
#
...
from arguments import ModelParams, PipelineParams, OptimizationParams
from tracker import GaussianTracker
from vgs_Gaussianmodel import VGSGaussianModel
try:
    from torch.utils.tensorboard import SummaryWriter
    TENSORBOARD_FOUND = True
except ImportError:
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    vgs_config = {
        "enable": True,
        "std_pos": 0.045,       # 位置扰动标准差
        "std_scale": 0.045,     # scale扰动标准差
        "frequency": 500,     # 每500轮扰动一次，0表示只扰动一次
        "max_steps": 2000,
    }
    gaussians = GaussianModel(dataset.sh_degree, opt.optimizer_type)
    #!！！
    '''add new'''
    tracker = GaussianTracker(model=gaussians, out_dir=os.path.join(dataset.model_path, "tracker_logs"), log_interval=100)
    tracker.initialize()
    '''add new'''
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        ##!!加入扰动
        if vgs_config["enable"] and iteration % vgs_config["frequency"] == 0 :
            xyz_pertured,scale_perturbed = gaussians.generate_perturbed_params(vgs_config["std_pos"],vgs_config["std_scale"])
            
        else :
            xyz_perturbed,scale_perturbed = None,None
        ...
        #! 修改render函数
        render_pkg = render(viewpoint_cam, gaussians, pipe, bg, use_trained_exp=dataset.train_test_exp, separate_sh=SPARSE_ADAM_AVAILABLE,override_xyz = xyz_perturbed,override_scaling = scale_perturbed)
...
if __name__ == "__main__":
    ...
    # All done
    print("\nTraining complete.")ns, checkpoint_iterations, checkpoint, debug_from):
